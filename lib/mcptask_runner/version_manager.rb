# frozen_string_literal: true

module McptaskRunner
  # VersionManager handles version incrementing by 0.1 increments
  # Reads and writes version from lib/mcptask_runner/version.rb
  class VersionManager
    VERSION_FILE = File.expand_path('version.rb', __dir__)

    def self.current_version
      McptaskRunner::VERSION
    end

    def self.increment_version!
      current = parse_version(current_version)
      new_version = increment_patch(current)
      old_version = current_version
      write_version(new_version)
      puts "[VersionManager] Version incremented: #{old_version} → #{new_version}"
      new_version
    end

    private

    def self.parse_version(version_string)
      # Parse "0.1.0" into [0, 1, 0]
      version_string.split('.').map(&:to_i)
    end

    def self.increment_patch(version_array)
      # version_array is [major, minor, patch]
      # Increment patch by 1 to achieve 0.1 increments
      # e.g., [0, 1, 0] → [0, 1, 1], [0, 1, 9] → [0, 2, 0], [0, 9, 9] → [1, 0, 0]
      major, minor, patch = version_array

      patch += 1
      if patch >= 10
        minor += 1
        patch = 0
      end

      if minor >= 10
        major += 1
        minor = 0
      end

      "#{major}.#{minor}.#{patch}"
    end

    def self.write_version(new_version)
      version_content = <<~RUBY
        module McptaskRunner
          VERSION = "#{new_version}"
        end
      RUBY

      File.write(VERSION_FILE, version_content)

      # Reload the version constant - need to force redefine the module
      Object.send(:remove_const, :McptaskRunner) if Object.const_defined?(:McptaskRunner)
      Kernel.load(VERSION_FILE)
    end
  end
end
